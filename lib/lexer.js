...
const KEYWORDS = {
  'tee':   TOKEN_TYPES.TEE,
  'drive': TOKEN_TYPES.DRIVE,
  'putt':  TOKEN_TYPES.PUTT,
};
...
export class Lexer {
  ...
  skipWhitespace() {
    while (!this.isAtEnd() && isWhitespace(this.peek())) this.advance();
  }
  ...
  skipComment() {
    ...
    this.errors.push(new LexerError('Unterminated block comment (missing */)', startLine, startCol));
  }

  readNumber(line, col) {
    let raw = '';
    while (!this.isAtEnd() && isDigit(this.peek())) raw += this.advance();
    return new Token(TOKEN_TYPES.NUMBER, parseInt(raw, 10), line, col);
  }

  readIdentifierOrKeyword(line, col) {
    let name = '';
    while (!this.isAtEnd() && isAlphaNum(this.peek())) name += this.advance();
  ...
  tokenize() {
    while (!this.isAtEnd()) {
      ...
      if (this.peek() === '/' && this.peek(1) === '*') { this.skipComment(); continue; }

      const line = this.line;
      const col  = this.column;
      const ch   = this.peek();

      if (isDigit(ch))      { this.tokens.push(this.readNumber(line, col)); continue; }
      if (isIdentStart(ch)) { this.tokens.push(this.readIdentifierOrKeyword(line, col)); continue; }
      ...
      switch (ch) {
        ...
        default:
          this.errors.push(new LexerError(
            `Unexpected character '${ch}' (ASCII ${ch.charCodeAt(0)})`, line, col
          ));
      }
    }
