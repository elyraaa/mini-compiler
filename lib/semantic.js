/**
 * SEMANTIC ANALYZER (Type Checker / Context Analysis)
 * 
 * Walks the AST and enforces semantic rules that cannot be
 * captured by the context-free grammar alone:
 * 
 *   1. All variables must be declared before use.
 *   2. No variable may be declared twice.
 *   3. Variables used in expressions should be initialized.
 *   4. Unused variables produce warnings (not errors).
 * 
 * Implements a single-scope Symbol Table (since the language
 * has no functions, blocks, or scoping constructs).
 * 
 * Theory: This corresponds to "attribute grammars" and 
 * "type systems" as described in:
 *   - Pierce, "Types and Programming Languages" (TAPL)
 *   - Dragon Book, Chapter 6
 */

// ─────────────────────────────────────────────
//  SEMANTIC ERROR / WARNING CLASSES
// ─────────────────────────────────────────────
